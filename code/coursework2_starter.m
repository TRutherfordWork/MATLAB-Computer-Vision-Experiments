...
% run('F:\University\Computer Science\Year 3\Computer Vision\Coursework 2\code\vlfeat-0.9.21-bin\vlfeat-0.9.21/toolbox/vl_setup')
...
switch lower(FEATURE)    
    ...
    case 'bag of sift'
        ...
        if ~exist('vocab.mat', 'file')
            ...
            vocab = build_vocabulary(train_image_paths, vocab_size, 0); %Also allow for different sift parameters
            save('vocab.mat', 'vocab')
        end
        
        % YOU CODE get_bags_of_sifts.m
        if ~exist('image_feats.mat', 'file')
            train_image_feats = get_bags_of_sifts(train_image_paths, 0); %Allow for different sift parameters
            test_image_feats  = get_bags_of_sifts(test_image_paths, 0); 
            save('image_feats.mat', 'train_image_feats', 'test_image_feats')
        end
    case 'spatial pyramids'
          if ~exist('vocab.mat', 'file')
              fprintf('No existing dictionary found. Computing one from training images\n')
              vocab = build_vocabulary(train_image_paths, vocab_size, 1);
              save('vocab.mat', 'vocab');
          end

          % YOU CODE spatial pyramids method
          if ~exist('image_feats.mat', 'file')
            train_image_feats = get_bags_of_sifts(train_image_paths, 1); %Allow for different sift parameters
            test_image_feats  = get_bags_of_sifts(test_image_paths, 1); 
            save('image_feats.mat', 'train_image_feats', 'test_image_feats')
          end
    case 'rgb bag of sifts'
          if ~exist('vocab.mat', 'file')
              fprintf('No existing dictionary found. Computing one from training images\n')
              vocab = build_rgb_vocabulary(train_image_paths, vocab_size, 0);
              save('vocab.mat', 'vocab');
          end
            
          % YOU CODE spatial pyramids method
          if ~exist('image_feats.mat', 'file')
            train_image_feats = get_rgb_bags_of_sifts(train_image_paths, 0); %Allow for different sift parameters
            test_image_feats  = get_rgb_bags_of_sifts(test_image_paths, 0); 
            save('image_feats.mat', 'train_image_feats', 'test_image_feats')
          end
    case 'hog'
        if ~exist('vocab.mat', 'file')
              fprintf('No existing dictionary found. Computing one from training images\n')
              vocab = build_hog_vocabulary(train_image_paths, vocab_size);
              save('vocab.mat', 'vocab');
          end
            
          % YOU CODE spatial pyramids method
          if ~exist('image_feats.mat', 'file')
            train_image_feats = get_bags_of_hogs(train_image_paths); %Allow for different sift parameters
